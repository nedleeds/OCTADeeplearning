...
class RetainBest:
    ...
    def step(self, metrics_valid):
        ...
        metrics_dict = dict()
        for selection in self.selection_metrics: #selection_metrics: loss, acc, f1 ...
            if metric_optimum[selection] == "min":
                metrics_dict[selection] = (
                    metrics_valid[selection] < self.best_metrics[selection] # best보다 작으면, 현재의 value로 update
                )
                self.best_metrics[selection] = min(
                    metrics_valid[selection], self.best_metrics[selection] # best_metric <- min(curr, pre_best)
                )
