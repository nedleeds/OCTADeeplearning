...
def inject(model, output_dir=None, backend='gcam', layer='auto', label=None, data_shape='default',
           ...
           return_score=False, replace=False, return_attention=False, cudnn=True, enabled=True, file_names=None):
    """
    ...
    medcam_dict['file_names'] = file_names
    medcam_dict['nib_info'] = None
...
@check_call
def forward(self, batch, label=None, mask=None, raw_input=None, nib_info=None):
    """
    ...
    if self.medcam_dict['enabled']:
        self.medcam_dict['nib_info'] = nib_info
...
def _assign_backend(backend, model, target_layers, postprocessor, retain_graph):
    ...
    print('medcam backed :', backend, end=' -> ')
    if backend == "gbp":
...
def _save_attention_map(self, attention_map, layer_output_dir, j, k, raw_input):
    ...
    idx = self.medcam_dict['counter']
    id_disease = self.medcam_dict['file_names'][idx].split('/')[-1]
    name = id_disease + "_" + str(idx) + "_" + str(j) + "_" + str(k)
    save_path = layer_output_dir + '/' + name
    
    print(f'{idx+1} - {id_disease:>6} attention map saved.')
    if self.medcam_dict['save_pickle']:
        self.medcam_dict['pickle_maps'].append(attention_map)
    if self.medcam_dict['save_maps']:
        medcam_utils.save_attention_map(filename=save_path, 
                                        attention_map=attention_map, 
                                        heatmap=self.medcam_dict['heatmap'], 
                                        raw_input=raw_input,
                                        nib_info=self.medcam_dict['nib_info'])
